...
export default function WatchCanvas({ scrollState, activeMaterial }: WatchCanvasProps) {
  // A couple hundred px of margin means the loop resumes just before the
  // canvas actually enters the viewport, so there's no visible pop-in.
  // frameloop="never" fully stops R3F's render loop while off-screen —
  // no per-frame damping/rotation math, no GPU draw calls — rather than just
  // hiding a canvas that's still silently rendering underneath.
  const [containerRef, inView] = useInViewport<HTMLDivElement>('200px 0px')
